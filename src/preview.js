...
function initPreviewPage() {
  ...
  if (sealIdElement && (!sealIdElement.textContent || sealIdElement.textContent === '2a24c622f84547ab')) {
    const randomId = (Math.random().toString(16).slice(2) + Math.random().toString(16).slice(2)).slice(0, 16);
...
function generateQRCode() {
  try {
    ...
    const compressed = LZString.compressToEncodedURIComponent(JSON.stringify(qrDataObj));

    // 创建完整的 URL
    const resultUrl = `https://jt.dogxi.me/result.html?d=${compressed}`;
